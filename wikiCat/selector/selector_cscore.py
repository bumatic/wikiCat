...
# from wikiCat.processor.pandas_processor import PandasProcessorGraph
import pandas as pd
import os
...
class SelectorCscore(GtGraphProcessor): #(GtGraphProcessor, PandasProcessorGtaph)
    ...
    def get_highest_cscores(self, cscore_type, n=100, cats_only=False, save=False):
        assert cscore_type == 'nodes' or cscore_type == 'edges' or cscore_type == 'events', 'Error. Pass one of the following types: nodes, edges, events.'

        df_nodes = self.load_nodes(os.path.join(self.nodes_location, self.nodes[0]), ['id', 'title', 'ns', 'cscore'])
        df_edges = self.load_edges(os.path.join(self.edges_location, self.edges[0]), ['source', 'target', 'type', 'cscore'])
        ...
        elif cscore_type == 'events':
            df_events = self.load_events(os.path.join(self.events_location, self.events[0]), ['revision', 'source', 'target', 'event', 'cscore'])
        ...
        outfile = os.path.join(self.graph.project.pinfo['path']['results'], 'cscore_lists',
...
'''
#ALTE IMPLEMENTIERUNG MIT PandasProcessor
from wikiCat.processor.gt_graph_processor import GtGraphProcessor
