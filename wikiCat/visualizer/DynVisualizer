...
class DynVisualizer(Visualizer):
    def __init__(self, graph):
        ...
        self.events_file = os.path.join(self.working_graph_path, self.data[self.working_graph]['source_events'])
    ...
    def resolve_ids(self, df):
        ...
        df = df[['time', 'gt_id', 'target', 'type', 'cscore']]
        df.columns = [['time', 'gt_id', 'target', 'event', 'cscore']]
    ...
    def process_drawing_properties(self, output_size=None, vertex_size=None, vertex_min=None, vertex_max=None,
        ...
        if vertex_text:
            self.set_drawing_properties(vertex_text=self.gt.vp.title)

        if vertex_font_size is not None:
            self.set_drawing_properties(vertex_font_size=vertex_font_size)
...
class ARFVid(DynVisualizer):
    ...
    def update_state(self):
        ...
        arf_layout(g, pos=pos, max_iter=100, dt=1e-4)
    ...
    def visualize(self, output_size=None, vertex_size=None, vertex_min=None, vertex_max=None, vertex_text=None,
        ...
        self.process_drawing_properties(output_size=output_size,
                                        ...
                                        edge_min=edge_min,
                                        edge_max=edge_max)
        pos = arf_layout(self.gt, max_iter=100, dt=1e-4)
        self.graph_generator = self.dynamic_graph_generator(self.gt)
        self.gt.clear_edges()
        win = GraphWindow(self.gt,
                          pos,
